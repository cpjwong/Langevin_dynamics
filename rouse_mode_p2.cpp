...
int main(int argc, char** argv)
{
	...
	for(int i=0;i<cn;i++){
		for(int j=0;j<t_step;j++){
			for(int k1=0;k1<ac;k1++){
				...
				for(int k2=0;k2<ac;k2++){
					sum1=sum1+x(k2+i*ac,j)*A_f(k1,k2,ac);
					sum2=sum2+y(k2+i*ac,j)*A_f(k1,k2,ac);
					sum3=sum3+z(k2+i*ac,j)*A_f(k1,k2,ac);
				}
		...
		for(int k=0;k<ac;k++){
			for(int j=0;j<t_step;j++){
				...
				for (int k1=0;k1<tau_size;k1++){
					c=c+(Xp1(k,k1)*Xp1(k,k1+j)+Xp2(k,k1)*Xp2(k,k1+j)+Xp3(k,k1)*Xp3(k,k1+j))/(mean_Xsq(k)*(double)(tau_size));//(Xp1(k,k1)*Xp1(k,k1)+Xp2(k,k1)*Xp2(k,k1)+Xp3(k,k1)*Xp3(k,k1))
				}
				corr(j,k)=c;
			}
		}
		...
		string file2="Xp2_";
		string file3="Xp3_";
		string file4="rouse_1_";
		string file5="mean_sq_X_";
		ofstream myfile;
